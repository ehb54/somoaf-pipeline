...
while ( $fid = shift ) {
    ...
    for my $fpdb ( @pdbs ) {
        ...
        if ( !$skipproc ) {
            ...
            log_flush();
            my $cmd = "cp $f $dir/ && cd $dir && $cdcmd $f && grep -v Workdir: CD_comp.out | perl -pe 's/ \\/srv.*SESCA\\// SESCA\\//' > $$p_config{sescadir}/${fpdbnoext}-sesca-cd.dat";
            run_cmd( $cmd, true );
...

        {
            ...
            my ( $fh, $ft ) = tempfile( "$$p_config{tempdir}/somocmds.XXXXXX", UNLINK => 0 );
            print $fh
                "threads 1\n"
                . "batch selectall\n"
                . "batch somo_o\n"
            ...
            ## make tar & zip files
            {
                ...
                my $cmd =
                    "cd $dir"
                    . " && cp $$p_config{pdbstage2}/$fpdb $$p_config{mmcifdir}/${fpdbnoext}.cif $$p_config{prdir}/${fpdbnoext}-pr.dat $$p_config{sescadir}/${fpdbnoext}-sesca-cd.dat $$p_config{csvdir}/${fpdbnoext}.csv ."
